...
std::string getexepath()
{
 // char result[ PATH_MAX ];
  //ssize_t count = readlink( "/proc", result, PATH_MAX );
    char cwd[1024];
...
bool load_data(std::vector<std::string> &list, std::string path){
    std::string input=path +"/"+ "list.txt";
    std::ifstream file_stream_pose(input.c_str());
...
bool GetDepthData( std::string &file_name, ushort *data) {
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
      ...
      std::cout<<"Path to directory to unpack all dataset: "<<path<<"\n";
      local_dir=path+"/Dataset/";
      if(!load_data(list, path)){
          std::cout<<"list.txt is not found"<<"\n";
          return 0;
    }else{
        
        for(int i=0;i<list.size();i++){

            sequence_name=list[i]+"/";
            std::string dir=local_dir+sequence_name;
            std::cout<<"Load sequence: "<<sequence_name<<"\n";
            std::cout<<"Path: "<<dir<<"\n";
            DataFromServerToLocal(sequence_name, dir);
            DataLocalProcess(dir);                       
        }
