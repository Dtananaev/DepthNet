...
function F_gt = SUN3Dflow(sequenceName, frameID, frameIDAnother)

    % this funciton compute a optical flow from the SUN3D depth map
    % input a sequence name with two frame IDs 
    % output an optical flow field
    ...
    if ~exist('frameID','var')
        frameID = 1;
    end
    
    if ~exist('frameIDAnother','var')
        frameIDAnother = 2;
    end

    % the root path of SUN3D
    % change it to local
    SUN3Dpath = '/misc/lmbraid11/tananaed/DepthNet/SUN3D/Dataset/';
    ...
    disp('extrinsics')
    disp(size(extrinsicsC2W)) 
    disp(extrinsicsC2W(:,:,1))
    % read time stamp
    ...
    [gridY, gridX] = ndgrid(1:480,1:640);
    try
        depth = depthRead(fullfile(fullfile(SUN3Dpath,sequenceName,'depthTSDF',depthFiles(IDimage2depth(frameID)).name)));
    catch
    ...
    valid = logical(XYZcamera(:,:,4));  valid = valid(:)'; valid = find(valid);
    
    XYZcamera = reshape(XYZcamera,[],4)';
    XYZcamera = XYZcamera(1:3,valid);

    
    XYZworld = transformPointCloud(XYZcamera,extrinsicsC2W(:,:,frameID));
        
    XYZcameraAnother = transformPointCloudInverse(XYZworld,extrinsicsC2W(:,:,frameIDAnother));   
    
    xyProject = K * XYZcameraAnother;
    
    projectDepth = zeros(480,640);
    projectDepth(valid) = xyProject(3,:);
    
    
    try
        depthAnother = depthRead(fullfile(fullfile(SUN3Dpath,sequenceName,'depthTSDF',depthFiles(IDimage2depth(frameIDAnother)).name)));
    catch
        depthAnother = depthRead(fullfile(fullfile(SUN3Dpath,sequenceName,'depth',depthFiles(IDimage2depth(frameIDAnother)).name)));
    end
    
    
    good3D = abs(projectDepth-double(depthAnother))<0.05; % a threshold for 0.05 meter
    
    validgood = find(good3D(valid));
    valid = valid(validgood);
    
    gridY = gridY(valid);
    gridX = gridX(valid);    
    
    xyProject = xyProject(1:2,validgood) ./ xyProject([3 3],validgood);
    
    u = xyProject(1,:) - gridX;
    v = xyProject(2,:) - gridY;
    
    % outlier removal
    stdu = std(u);  medu = median(u);
    stdv = std(v);  medv = median(v);
    
    thresholdStd = 3;
    
    v2 = medu - thresholdStd*stdu < u & u< medu + thresholdStd*stdu & medv - thresholdStd*stdv < v & v< medv + thresholdStd*stdv ;
    u = u(v2);
    v = v(v2);
    valid = valid(v2);
    
    
    F_gt = zeros(480,640,3);
    
    F_gt(valid) = u;
    F_gt(valid+640*480) = v;
    F_gt(valid+640*480*2) = 1;
    
    figure
    
    subplot(2,3,1);
    imshow(imageRead(fullfile(fullfile(SUN3Dpath,sequenceName,'image',imageFiles(frameID).name))));
    title([sequenceName ':' num2str(frameID)], 'interpreter', 'none')
    
    subplot(2,3,4);
    imshow(imageRead(fullfile(fullfile(SUN3Dpath,sequenceName,'image',imageFiles(frameIDAnother).name))));
    title([sequenceName ':' num2str(frameIDAnother)], 'interpreter', 'none')
    
    subplot(2,3,3);
    imagesc(projectDepth); axis equal; axis tight; axis off
    title('projected depth');
    
    
    subplot(2,3,2);
    imagesc(F_gt(:,:,1)); axis equal; axis tight; axis off
    title('u flow');
    colorbar
    
    subplot(2,3,5);
    imagesc(F_gt(:,:,2)); axis equal; axis tight; axis off
    title('v flow')
    colorbar
    
    subplot(2,3,6);
    imshow(flow_to_color(F_gt,14));
    title('flow')
end
...
function XYZtransform = transformPointCloudInverse(XYZ,Rt)
    XYZtransform = inv(Rt(1:3,1:3)) * (XYZ - repmat(Rt(1:3,4),1,size(XYZ,2)));
end
